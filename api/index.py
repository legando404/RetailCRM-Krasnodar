...
site = os.getenv('site')#= 'krasnodar-novers-ru'
...
async def get_mail(username, password, imap_server):
    ...
    with MailBox(imap_server).login(username, password, initial_folder='Novers Краснодар') as mailbox:
        print('fetching...')
        exists = mailbox.folder.exists('Novers Краснодар/INBOX|Krasnodar')
        if not exists:
            mailbox.folder.create('Novers Краснодар/INBOX|Krasnodar')
       
        # Берём все письма, вне зависимости от прочитанности
        for msg in mailbox.fetch():
            mailbox.move(msg.uid, 'Novers Краснодар/INBOX|Krasnodar') 
